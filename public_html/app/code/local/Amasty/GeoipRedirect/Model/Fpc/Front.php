    ...
    public function extractContent($content)
    {
        ...
        if ($this->getDbConfig('amgeoipredirect/general/enable', $currentStoreId) && $this->redirectAllowed) {
            ...
            if (!$getAmYetRedirectUrl && $this->getDbConfig('amgeoipredirect/country_url/enable_url')) {
                $urlMapping = unserialize($this->getDbConfig('amgeoipredirect/country_url/url_mapping', $currentStoreId));
                ...
                foreach ($allStores as $store) {
                    ...
                    if ($country && $countries && strpos($countries, $country) !== false
                        ...
                        if ($storeId != $currentStoreId) {
                            ...
                            setcookie('AmYetRedirectStore', 1);
                            setcookie('am_current_store_url', trim($currentStoreUrl, '/'));
            ...
            if (!$getAmYetRedirectCurrency && $this->getDbConfig('amgeoipredirect/country_currency/enable_currency')) {
                $currencyMapping = unserialize($this->getDbConfig('amgeoipredirect/country_currency/currency_mapping', $currentStoreId));
                foreach ($currencyMapping as $value) {
                    ...
                    if ($checkCountry && $currentCurrencyCode != $value['currency']) {
                        setcookie('AmYetRedirectCurrency', 1);
                        setcookie('currency', $value['currency']);
...
//        $ip = '213.184.225.37';//Minsk
        if ($this->getDbConfig('amgeoip/import/location') && $this->getDbConfig('amgeoip/import/block')) {
            ...
            if (!empty($longIP)) {
                ...
                $adapter = $resource->getConnection('core_read');
                $select = $adapter->select()
                    ->from(array('l' => $resource->getTableName('amgeoip/location')))
                    ->join(
                        array('b' => $resource->getTableName('amgeoip/block')),
                        'b.geoip_loc_id = l.geoip_loc_id',
                        array()
                    )
                    ->where('? >= b.start_ip_num', $longIP)
                    ->order('start_ip_num DESC')
                    ->limit(1);

                if ($res = $adapter->fetchRow($select))
                    $this->setData($res);
            }
        }
