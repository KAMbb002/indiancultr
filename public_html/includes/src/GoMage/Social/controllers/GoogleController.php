...
class GoMage_Social_GoogleController extends GoMage_Social_Controller_Social {
	...
	public function loginAction() {
		...
		if (!Mage::getSingleton('core/session')->getData('gs_url') && $this->getRequest()->getParam('gs_url', '')) {
			...
			if ($client->getAccessToken()){								
					...
					if ($profile && is_array($profile) && isset($profile['id'])) {
			        	...
			        	if ($customer && $customer->getId()){
							...
							$this->updateGoogleData($plus_data,$customer->getId());
